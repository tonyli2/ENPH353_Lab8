...
class Net(nn.Module):
    ...
    def __init__(self, obs_size, hidden_size, n_actions):
        ...
        self.net = nn.Sequential(
            nn.Linear(obs_size, hidden_size), # Obs_size is num of inputs, outputs hidden_size number of outputs
            nn.ReLU(),
            nn.Linear(hidden_size, n_actions) # Hidden_size is num of inputs, outputs n_actions number of outputs
        )
...
def iterate_batches(env, net, batch_size):
    ...
    #   * theta
    #   * theta_dot
    obs = env.reset() # Stores state info into obs
    ...
    while True:
        ...
        action = np.random.choice(len(act_probs), p=act_probs) # p = probability of each action
...
def filter_batch(batch, percentile):
    ...
    # For every Episode in batch, map the Episodes rewards to a list of rewards
    ...
    # Takes a list and finds the value at which "percentile" of the data falls below that value
...
if __name__ == '__main__':
    ...
    # expresion
    objective = nn.CrossEntropyLoss() # Loss function used for classification, output is probability
    optimizer = optim.Adam(params=net.parameters(), lr=0.01) # we pass networks parameters as 
    # things the optimizer may change. Set lr the learning rate at 0.01
    ...
    for iter_no, batch in enumerate(iterate_batches(env, net, BATCH_SIZE)):
        ...
        obs_v, acts_v, reward_b, reward_m = filter_batch(batch, 0.3) # identify the episode in top PERCENTILE
        ...
        # episodes
        # forward pass on each observation
        action_scores_v = net(obs_v) # dont call net.forward() as the function call is better for debugging
        ...
        # backward pass calculates the gradient of the output wrt each W and b
        loss_v.backward() # backward pass is called on loss tensor

        # adjust weights, bias, etc. based on gradients
        optimizer.step()
